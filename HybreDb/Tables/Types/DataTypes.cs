...
namespace HybreDb.Tables {
    ...
    ///     The extended classes of DataType, must have both the empty constructor as well as the constructor with an binary
    ///     reader
    /// </summary>
    ...
    public static class DataTypes {
        ...
        /// <summary>
        ///     Create the object implementation corresponding to the DataTypes, with a given BinaryReader holding the actual data
        ///     of that type
        /// </summary>
        public static IDataType CreateType(this Types t, BinaryReader rdr) {
            switch (t) {
                case Types.Number:
                    return new Number(rdr);
                case Types.Text:
                    return new Text(rdr);
                case Types.DateTime:
        ...
        public static IDataType CreateType(this Types t, object d) {
            switch (t) {
                case Types.Number:
                    return new Number(Convert.ToInt32(d));
                case Types.Text:
                    return new Text((string) d);
                case Types.DateTime:
