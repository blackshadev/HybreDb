...
namespace HybreDb.Actions.Result {
    ...
    /// <summary>
    ///     Serialises the Relational dataset by adding the relation as property and keeping track of the rows which are needed
    ///     from the tables.
    ...
    public class HybreRelationDataSerialiser : JsonConverter {
        public override void WriteJson(JsonWriter writer, object value, JsonSerializer serializer) {
            ...
            serializer.Serialize(writer,
                (new HybreDataResult(rel.Relation.Source, rel.Relation.Source.GetData(srcNums))));

            writer.WritePropertyName("destinationTable");
            serializer.Serialize(writer,
                new HybreDataResult(rel.Relation.Destination, rel.Relation.Destination.GetData(dstNums)));
