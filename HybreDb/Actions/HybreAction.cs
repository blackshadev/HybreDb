...
namespace HybreDb.Actions {
    ...
    public static class HybreAction {
        ...
        public static IHybreAction Parse(string json) {
            ...
            string cName = "HybreDb.Actions.HybreAction" + char.ToUpper(m[0]) + m.Substring(1);
            Type t = Type.GetType(cName, true, true);
        ...
        /// <param name="data">
        ///     Dictionary of the data. The key is the columnName of the data and the value is the data of that
        ///     column
        /// </param>
        ...
        public static IDataType[] ParseData(Table t, Dictionary<string, object> data) {
            if (t.Columns.Length != data.Count)
                throw new ArgumentException("Invalid number of data given, expected " + t.Columns.Length + " got " +
                                            data.Count);

            var row = new IDataType[t.Columns.Length];
            foreach (var d in data) {
                int iX = t.Columns.GetIndex(d.Key);
        ...
        public static IDataType[] ParseData(Relation r, Dictionary<string, object> data) {
            if (r.Table.Columns.Length - 3 != data.Count)
                throw new ArgumentException("Invalid number of data given, expected " + (r.Table.Columns.Length - 3) +
                                            " got " + data.Count);

            var row = new IDataType[data.Count];
            foreach (var d in data) {
                int iX = r.Table.Columns.GetIndex(d.Key);
        ...
        public static IDataType[] ParseDataWithRel(Relation r, Dictionary<string, object> data) {
            if (r.Table.Columns.Length - 1 != data.Count)
                throw new ArgumentException("Invalid number of data given, expected " + (r.Table.Columns.Length - 1) +
                                            " got " + data.Count);
        ...
        /// <returns>Result of the action</returns>
        /// <remarks>
        ///     This function catches exceptions thrown in execution of the action, these exceptions are returned as a
        ///     instance of HybreResult by HybreError
        /// </remarks>
